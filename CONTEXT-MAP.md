...
- **Slate → Site & Infrastructure / Home Server**: Slate (formerly Divvy) follows Inventory's hosting pattern exactly — PWA at `slate.mattrandell.com` (Cloudflare Pages, Terraform-managed), with `divvy.mattrandell.com` kept as a 301 redirect so old share links still resolve. PocketBase backend stays at `divvy-api.mattrandell.com` on the Home Server via Coolify through the single Cloudflare Tunnel (the backend host and `/api/divvy/*` routes keep the old name so installed PWAs don't break). Receipt OCR calls the Claude API from a PocketBase hook. See [slate/docs/adr/0001](./slate/docs/adr/0001-no-accounts-link-token-access.md) and [0002](./slate/docs/adr/0002-receipt-ocr-via-claude.md).
